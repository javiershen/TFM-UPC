...
function createChannel() {
  ...
  if [ $CHANNEL_NAME == "channel2"  ]; then
    scripts/createChannelTwo.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
  else
    scripts/createChannel.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
  fi
}

function createChannelTwo() {
  # Bring up the network if it is not already up.
  bringUpNetwork="false"
...
function deployCCTwo() {
  scripts/deployCCTwo.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.pharma.com docker_peer0.org1.pharma.com docker_peer0.org2.pharma.com
...
if [ "$MODE" == "up" ]; then
  ...
  createChannel
elif [ "$MODE" == "createChannelTwo" ]; then
  infoln "Creating channel '${CHANNEL_NAME}'."
  infoln "If network is not up, starting nodes with CLI timeout of '${MAX_RETRY}' tries and CLI delay of '${CLI_DELAY}' seconds and using database '${DATABASE} ${CRYPTO_MODE}"
  createChannelTwo
elif [ "$MODE" == "down" ]; then
